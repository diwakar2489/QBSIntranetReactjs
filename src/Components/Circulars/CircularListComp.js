...
function CircularListComp() {
    const pagination = useRef();
    const [perPage, setPerpage] = useState();
    const [circularlist, setCircularlist] = useState([]);
    const [totalitem, setTotalitem] = useState();
    const setPage = ({ selected }) => {
        getcircularlist(selected + 1);
    }
    useEffect(()=>{
        getcircularlist()
    },[])
    const getcircularlist = async (pagenumber) => {
        let SERVER_URL = process.env.REACT_APP_BASE_URL;

        const token = JSON.parse(localStorage.getItem('key'));
        const response = await fetch(SERVER_URL + `api/circular?page=${pagenumber}`,
            {
                method: 'GET',
                headers: {
    ...
    return (
        <div className="row-bg">
            <div className="table_search_wrp">
                <Link to="/addcircular" className="btn_style2 float-end"><img src="assets/images/plus.png" /> Add Circular</Link>
                ...
                <tbody>
                {
                        circularlist?.map((item, ind) => {
                            return <tr key={ind}>
                                ...
                                <td>
                                <Link to={'/editcircular?id='+item.id} class="btn-style1">
...
export default CircularListComp
